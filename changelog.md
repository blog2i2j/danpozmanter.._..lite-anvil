...
* Lua 5.4 VM embedded via `mlua` (vendored, no system liblua5.4-dev required)
* SDL3 window and event loop via `sdl3` crate
* FreeType2 font loading and glyph rasterization via `freetype` crate
* PCRE2 regex via `pcre2` crate (UTF + UCP always enabled)
* Cross-platform directory monitoring via `notify` crate (replaces 6 C backends)
* Render cache with dirty-rect blitting — pixel-exact port of rencache.c
