...
fn translate_event_native(
    ...
    if t == SDL_EVENT_WINDOW_RESIZED {
        ...
        return Some(EditorEvent::Resized { w: w as f64, h: h as f64 });
    }
    ...
    if t == SDL_EVENT_KEY_DOWN || t == SDL_EVENT_KEY_UP {
        let (scancode, keycode, keymod, repeat) = unsafe {
            (event.key.scancode, event.key.key, event.key.r#mod, event.key.repeat)
        };
        ...
        return if t == SDL_EVENT_KEY_DOWN {
            Some(EditorEvent::KeyPressed { key: name, modifiers })
        } else {
            Some(EditorEvent::KeyReleased { key: name, modifiers })
        };
    }
    ...
    if t == SDL_EVENT_MOUSE_BUTTON_DOWN || t == SDL_EVENT_MOUSE_BUTTON_UP {
        ...
        let button = match btn {
            1 => MB::Left, 2 => MB::Middle, 3 => MB::Right, 4 => MB::X1, 5 => MB::X2,
            _ => return None,
        };
    ...
    if t == SDL_EVENT_MOUSE_MOTION {
        ...
        loop {
            ...
            if !unsafe { SDL_PollEvent(&mut next_e) } { break; }
        ...
        return Some(EditorEvent::MouseMoved {
            x: fx as f64 * scale_x, y: fy as f64 * scale_y,
            dx: frx as f64 * scale_x, dy: fry as f64 * scale_y,
        });
    }
    if t == SDL_EVENT_MOUSE_WHEEL {
        let (wx, wy, dir) = unsafe { (event.wheel.x, event.wheel.y, event.wheel.direction) };
        let (vy, vx) = if dir == SDL_MouseWheelDirection::FLIPPED { (-wy, -wx) } else { (wy, wx) };
        return Some(EditorEvent::MouseWheel { x: -vx as f64, y: vy as f64 });
    }
    if t == SDL_EVENT_DROP_FILE {
        let data_ptr = unsafe { event.drop.data };
        if data_ptr.is_null() { return None; }
        let filename = unsafe { CStr::from_ptr(data_ptr) }.to_string_lossy().into_owned();
